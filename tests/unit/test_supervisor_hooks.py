"""Tests for supervisor hook checks in send_event.py.
...
class TestLeaderDoingTooMuch:
    ...
    def test_agent_call_resets_counter(self, tmp_path):
        """Agent调用应重置计数器。"""
        patches = _use_temp_state(str(tmp_path))
        with patches[0], patches[1]:
            ...
            # 积累一些调用
            for _ in range(7):
                _check_leader_doing_too_much(bash_event)

            # Agent调用重置计数器
            result = _check_leader_doing_too_much(agent_event)
            ...
            # 重新积累不应立即warning
            result = _check_leader_doing_too_much(bash_event)
            assert result is None

    def test_team_create_resets_counter(self, tmp_path):
        """TeamCreate调用也应重置计数器。"""
        patches = _use_temp_state(str(tmp_path))
        with patches[0], patches[1]:
            ...
            result = _check_leader_doing_too_much(create_event)
            assert result is None

            state = _load_supervisor_state()
            assert state.get("leader_consecutive_calls", 0) == 0

    def test_send_message_resets_counter(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            result = _check_leader_doing_too_much(msg_event)
            assert result is None

            state = _load_supervisor_state()
            assert state.get("leader_consecutive_calls", 0) == 0

    def test_empty_tool_name_no_warning(self, tmp_path):
        """空tool_name时不应处理。"""
        patches = _use_temp_state(str(tmp_path))
        with patches[0], patches[1]:
    ...
    def test_counter_persists_across_calls(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            _check_leader_doing_too_much(event)
            _check_leader_doing_too_much(event)
            _check_leader_doing_too_much(event)
...
class TestTeamHasPermanentMembers:
    ...
    def test_team_without_members_warning(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            result = _check_team_has_permanent_members(post_event)
            ...
            for i in range(1, _TEAM_WITHOUT_MEMBERS_THRESHOLD):
                result = _check_team_has_permanent_members(pre_event)
                assert result is None, f"Unexpected warning at call {i}"
    ...
    def test_agent_call_after_team_create_no_warning(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            _check_team_has_permanent_members(post_event)
            ...
            result = _check_team_has_permanent_members(agent_event)
            assert result is None
    ...
    def test_no_team_create_no_warning(self, tmp_path):
    ...
    def test_warning_only_fires_once(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            for _ in range(_TEAM_WITHOUT_MEMBERS_THRESHOLD):
                _check_team_has_permanent_members(pre_event)

            # 之后不应再warning（已重置）
            result = _check_team_has_permanent_members(pre_event)
            assert result is None

    def test_post_tool_use_non_team_create_ignored(self, tmp_path):
...
class TestNormalFlowNoWarning:
    ...
    def test_normal_flow_no_warning(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            for tool in ["Read", "Bash", "Read"]:
                ...
                r1 = _check_leader_doing_too_much(event)
                r2 = _check_team_has_permanent_members(event)
                assert r1 is None
                assert r2 is None
    ...
    def test_team_create_then_agent_no_warning(self, tmp_path):
        ...
        with patches[0], patches[1]:
            ...
            r = _check_team_has_permanent_members(post_event)
            assert r is None
            ...
            r = _check_team_has_permanent_members(agent_event)
            assert r is None
            ...
            for _ in range(5):
                r = _check_team_has_permanent_members(bash_event)
                assert r is None
